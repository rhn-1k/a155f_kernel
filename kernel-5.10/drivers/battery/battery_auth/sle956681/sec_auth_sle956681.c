...
static int get_heatmap(int *heatmap, int size)
{
	...
	convert_heatmap(read_hmap, heatmap, size);
 ...
 * */
static int authon_probe(struct platform_device *pdev)
{
 ...
 *************************************************************************/
	...
	platform_set_drvdata(pdev, authon_driver);
...
static int authon_suspend(struct device *dev)
{
	return 0;
}

static int authon_resume(struct device *dev)
{
	return 0;
}

static void authon_shutdown(struct platform_device *pdev)
{

}

/**
 * @brief Remove Authenticate On module driver
 * */

static int authon_remove(struct platform_device *pdev)
{
...
#ifdef CONFIG_OF
static struct of_device_id authon_dt_ids[] = {
	{ .compatible = "samsung,sec_auth_sle956681" },
	{ }
};
MODULE_DEVICE_TABLE(of, authon_dt_ids);
#endif /* CONFIG_OF */

static const struct dev_pm_ops authon_pm_ops = {
	.suspend = authon_suspend,
	.resume = authon_resume,
};

static struct platform_driver authon_driver = {
	.driver = {
		.name = "sec-authon",
		.owner = THIS_MODULE,
		.pm = &authon_pm_ops,
#ifdef CONFIG_OF
		.of_match_table = authon_dt_ids,
#endif
	},
	.probe = authon_probe,
	.remove = authon_remove,
	.shutdown = authon_shutdown,
};

static int __init authon_driver_init(void)
{
	return platform_driver_register(&authon_driver);
}

static void __exit authon_driver_exit(void)
{
	platform_driver_unregister(&authon_driver);
}
