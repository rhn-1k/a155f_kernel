...
{
	if (data->is_irq_active && !data->check_abnormal_working && !data->skip_data) {
...
static void sx9385_initialize_register(struct sx9385_p *data)
{
	...
	for (i = 0; i < data->num_of_channels; i++) {
		...
		if (ret < 0) {
			...
			data->ch[i]->detect_threshold = ((u16)val * (u16)val) >> 1; // (/ 2)
			data->ch[i]->release_threshold = ((u16)val * (u16)val) >> 1; // (/ 2)
			val = 0;
...
static void sx9385_get_refcap(struct sx9385_p *data)
{
	...
	for (idx = 0; idx < data->num_of_refs; idx++) {
		...
		cuseful = (use[idx] * data->again_r[idx]) / (data->dgain_r[idx] << 15);// (32768 * data->dgain_r[idx])
...
static void sx9385_get_data(struct sx9385_p *data)
{
	...
	for (i = 0; i < data->num_of_channels; i++) { // sensing
		msb = lsb = 0;
...
*/
		coffset = ((msb & 0xFE) >> 1) * 38000 + (((msb & 0x01) << 8) + lsb) * 125;
		// cuseful = (use[i] * data->ch[i]->again_m) / (32768 * data->ch[i]->dgain_m);
		cuseful = (use[i] * data->ch[i]->again_m) / (data->ch[i]->dgain_m << 15);
		data->ch[i]->capMain = coffset + cuseful;
		...
		avg[i] = (s16)(msb & 0x03)<<14 | mid<<6 | lsb >> 2;
		...
		msb = lsb = 0;
		sx9385_i2c_read(data, reg_dlt_msb[i],   &msb);
		sx9385_i2c_read(data, reg_dlt_msb[i]+1, &lsb);
		dlt_hex[i] = msb | lsb;
		dlt[i] = ((s16)msb<<8 | lsb) >> 4;// (/16)
	}
						...
						struct device_attribute *attr, char *buf)
{
	...
	if (sx9385_i2c_read(data, REG_PROX_CTRL5_PH2, &th_buf) < 0)
		...
		threshold = ((u32)th_buf * (u32)th_buf) >> 1; // (/ 2)

	if (sx9385_i2c_read(data, REG_PROX_CTRL4_PH2, &hyst) < 0)
						...
						struct device_attribute *attr, char *buf)
{
	...
	if (sx9385_i2c_read(data, REG_PROX_CTRL4_PH4, &hyst) < 0)
		GRIP_ERR("detect threshold update2 fail\n");
	else
		hyst = (hyst & 0x30) >> 4;
...
static int sx9385_check_dependency(struct device *dev, int ic_num)
{
	...
	if (vdd_retry_cnt && !(vdd_retry_cnt % 50))
		pr_info("[GRIP_%s] %s dvdd_vreg_name %s (%d)\n", grip_name[ic_num], __func__, dvdd_vreg_name, vdd_retry_cnt);
				...
				unsigned long action, void *hall_data)
{
	...
	if (action == HALL_ATTACH && data->init_done == ON) {
		GRIP_INFO("%s attach\n", hall_notifier->name);
		schedule_work(&data->cal_work);
	}
