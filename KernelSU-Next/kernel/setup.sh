...
perform_cleanup() {
    ...
    if [ -d "$GKI_ROOT/Wild_KSU" ]; then
        rm -rf "$GKI_ROOT/Wild_KSU" && echo "[-] Wild_KSU directory deleted."
    fi
}
...
setup_kernelsu() {
    ...
    if [ "$(git status | grep -Po 'v\d+(\.\d+)*' | head -n1)" ]; then
        git checkout wild && echo "[-] Switched to wild branch."
    fi
    ...
    cd "$DRIVER_DIR"
    ln -sf "$(realpath --relative-to="$DRIVER_DIR" "$GKI_ROOT/Wild_KSU/kernel")" "kernelsu" && echo "[+] Symlink created."
